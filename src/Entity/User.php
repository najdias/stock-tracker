...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
     ...
     */
    #[ORM\OneToMany(targetEntity: StockLog::class, mappedBy: 'user_id', orphanRemoval: true)]
    ...
    public function addStockLog(StockLog $stockLog): static
    {
        if (!$this->stockLogs->contains($stockLog)) {
            $this->stockLogs->add($stockLog);
            $stockLog->setUserId($this);
        }

        return $this;
    }

    public function removeStockLog(StockLog $stockLog): static
    {
        if ($this->stockLogs->removeElement($stockLog)) {
            // set the owning side to null (unless already changed)
            if ($stockLog->getUserId() === $this) {
                $stockLog->setUserId(null);
            }
        }
